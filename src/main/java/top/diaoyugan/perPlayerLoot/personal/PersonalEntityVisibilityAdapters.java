    ...
    public static PersonalEntityVisibilityAdapter create(final PerPlayerLoot plugin, final LootStorage storage) {
        if (!Bukkit.getPluginManager().isPluginEnabled("ProtocolLib")) {
            plugin.getLogger().warning(
                "ProtocolLib is not installed or not enabled. Personal item-frame and suspicious-block drops are disabled."
            );
            return null;
        }
        ...
        try {
            ...
            plugin.getLogger().log(
                Level.SEVERE,
                "Could not hook ProtocolLib. Personal item-frame and suspicious-block drops are disabled.",
                exception
            );
            return null;
        }
